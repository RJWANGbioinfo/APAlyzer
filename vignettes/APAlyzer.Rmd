...
Start from APAlyzer v1.2, we provides a new fouction called `APAVolcano` for 
users to plot their RED results using volcano plots. In the plot, users can also 
label the top genes using `top=` or a set of specific gene using `markergenes=`,
for example:
```{r eval=FALSE}
APAVolcano(test_3UTRmuti, PAS='3UTR', Pcol = "pvalue", top=5, main='test plot')
``` 
...
In addtion to volcano plots, APA comparison result can be also plotted using either boxplots or violin plots or 
